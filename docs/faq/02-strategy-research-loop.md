...
- Required capital ~ `max_drawdown x ~1.5` + margin buffer (often plan for *two* bad drawdowns early).
...
| Volatility (ATR) | Size so a kxATR stop ~ your $ risk | Fits options/underlying vol regimes later |
